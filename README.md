...
ts-poet is a TypeScript code generator based on [template literals](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals).

The primary goal is for generated code to be beautiful and pleasant to read: no warnings and no weird formatting.

It's key benefits over raw template literals are:

1. The `import` stanza is auto-generated with only actually-used symbols.

   This can dramatically clean up generated code, but avoiding "unused import" warnings, and the logic to conditionally output only actually-used symbols is fairly tedious to implement by hand.

2. `code"..."` literals can be built separately and then easily joined/combined together into a single output file.

   This facilitates decomposing code generates from "a single giant template" into.

3. Code is auto-formatted with dprint with prettier-esque settings (see [dprint vs prettier](#dprint-vs-prettier)).

4. Performance hints to avoid needlessly formatting unchanged output.

5. Includes any other conditional output (see later), as/if needed.
...
ts-poet originally used prettier to format its output.

However, because code generators are often invoked repetitively, and generate a potentially large amount of output, the cost of constantly prettier-izing the output became a large bottleneck for several of our projects.

To mitigate this, ts-poet switched to [dprint](https://dprint.dev/) (specifically [dprint-node]( (using your local `.prettierrc` if it exists))) which is a Rust-based formatter used by Deno.

The intent is for projects to still use Prettier for their day-to-day handwritten code (which is what we do), but to leverage dprint's extremely fast performance to generate "basically prettier-ish" output, such that readers should generally not be able to tell the difference. 

Granted, dprint and prettier output is not 100% the same, so we recommended `.prettierignore`-ing any files that ts-poet generates.
