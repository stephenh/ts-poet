    ...
    files.map(async (file) => {
      ...
      if (file.overwrite) {
        if (!file.hash) {
          ...
          await fs.writeFile(path, await contentToString(file, toStringOpts));
        } else {
          // Create a hash of the unformatted option (to avoid the cost of formatting)
          const hash = sha1(contentToUnformattedString(file, toStringOpts));
          if (exists) {
          ...
          const formatted = await contentToString(file, toStringOpts);
...
function contentToString(file: CodegenFile, toStringOpts: ToStringOpts): Promise<string> {
  if (typeof file.contents === "string") {
    return Promise.resolve(file.contents);
  }
  // Do a lazy/by-hand deep merge of toStringOpts and file.toStringOpts
  return file.contents.toStringAsync({
    path: file.name,
    ...toStringOpts,
    ...file.toStringOpts,
    formatOptions: {
      ...toStringOpts.formatOptions,
      ...file.toStringOpts?.formatOptions,
    },
  });
}

function contentToUnformattedString(file: CodegenFile, toStringOpts: ToStringOpts): string {
  if (typeof file.contents === "string") {
