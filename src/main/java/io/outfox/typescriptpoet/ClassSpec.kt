...
private constructor(
      ...
      it.parameters.emit(codeWriter, rest = it.restParameter) { param, isRest ->
        ...
        if (property != null && !isRest) {
          ...
          if (property.modifiers.none {
            it.isOneOf(Modifier.PUBLIC, Modifier.PRIVATE, Modifier.PROTECTED, Modifier.READONLY)
          }) {
  ...
  companion object {
    ...
    @JvmStatic
    fun builder(name: TypeName) = Builder(name.reference(null))
