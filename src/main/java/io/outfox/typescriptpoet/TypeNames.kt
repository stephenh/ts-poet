...
sealed class TypeName {
  ...
  internal constructor(
     val members: List<Anonymous.Member>
  ) : TypeName() {
  ...
  companion object {
    ...
    fun anyType(name: String): Any {
      ...
      if (idx != -1) {
        ...
        val imported = SymbolSpec.from("${usage.split('.').first()}${name.substring(idx)}")
    ...
    fun unionBound(type: String, keyOf: Boolean = false): Bound {
      return unionBound(TypeName.anyType(type), keyOf)
    }
    ...
    fun unionBound(type: TypeName, keyOf: Boolean = false): Bound {
      return bound(type, Combiner.UNION, if (keyOf) Bound.Modifier.KEY_OF else null)
    }
    ...
    fun intersectBound(type: String, keyOf: Boolean = false): Bound {
      return intersectBound(TypeName.anyType(type), keyOf)
    }
    ...
    fun anonymousType(vararg members: Pair<String, TypeName>): Anonymous {
      return Anonymous(members.map { Anonymous.Member(it.first, it.second) })
    }
